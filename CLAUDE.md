...
The `Page` type in `RouterContext` is the union of these path strings. Always use `Page` (not bare `string`) when navigating — `ProductSection.learnMoreHref` accepts `Page | undefined` for this reason.
...
All strings live in [src/i18n/translations.ts](src/i18n/translations.ts). Add every new string under all three language keys. The `MonitorsPage` translations live directly on the translations object under a `monitors` key.
...
├── images/                # Product images (laptop.png, monoblock.png, pc.png, monitor.png)
...
- **Only two products have detail pages** — `noutbuklar` (`/laptops`) and `monitorlar` (`/monitors`) have `learnMoreHref` wired in `BASE_PRODUCTS`; `monobloklar` and `cases` leave it undefined so the "Learn More" button is intentionally inert for those.

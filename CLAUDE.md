...
The Home page, all seven product pages, the About page, and the blog are all powered by Strapi v5. The client lives in [src/lib/strapi.ts](src/lib/strapi.ts); in every case CMS data takes priority and missing/failed CMS data falls back to hard-coded translations silently (`.catch()` resolves to `null`/`[]`).

- **Home page**: `useHomePageCms()` (from [src/lib/useProductPageCms.ts](src/lib/useProductPageCms.ts)) fetches the singleton `home-page` endpoint — `hero`, `categories`, `trust`, `buildpc` are each a JSON blob merged over the matching translation object in `Hero`, `CategoryGrid`, `TrustSection`, `CTASection`. `getProducts(locale)` separately fetches the `site-products` collection for the 4 `ProductSection` cards; `category_id` matches `BASE_PRODUCTS` ids (`noutbuklar`, `monobloklar`, `cases`, `monitorlar`). Navbar and Footer are NOT CMS-backed (shared chrome across every page, sourced from `translations.ts` only).
- **Product pages**: `useProductPageCms(slug)` fetches the `product-pages` collection by `slug` (`monitors` | `laptops` | `aios` | `nova` | `matrix` | `optima` | `cases`) and returns hero copy, `models` (lineup cards with image/video), `spec_categories`, and an `extra` JSON blob covering every other section (features, cta, bento, display/back/apps/connectivity/cam, etc.) — merged over translations in each page component. Pair `spec_categories` with `cmsToSpecCategories(cms)` to convert it into the `SpecsSection` `categories` prop shape.
- **About page**: `useAboutPageCms()` fetches the singleton `about-page` endpoint — hero, story, stats, values, milestones, founder info/image, plus an `extra` JSON blob (mission, what-we-do, why-choose, history, revenue, etc.).
